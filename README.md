...
- 当前 Hostdzire-LA 部署使用服务端代理模式：网页可以填写/保存 sub2api 地址、Bearer Token、分组、代理、优先级、倍率、WS mode 和自动透传，但实际请求仍由服务器 BFF 发起。
...
- 点击“保存配置”会把 sub2api 地址、Bearer Token、绑定设置、WS mode 和自动透传保存到服务器；点击导入/刷新时，浏览器只请求同源 `/token-manager/api/*`，真正的 sub2api 请求由服务器 BFF 完成。
